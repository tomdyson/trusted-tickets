...
class EventAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ('Bank Details', {
            'fields': ('bank_account_name', 'sort_code', 'account_number')
        }),
        ('Email Configuration', {
            'fields': ('contact_email', 'admin_notification_emails', 'confirmation_email_subject', 'confirmation_email_body'),
    ...
    def booking_count(self, obj):
        ...
        return format_html('<a href="/admin/tickets/booking/?event__id__exact={}">{}</a>', obj.id, count)
    booking_count.short_description = "Bookings"

    def total_revenue(self, obj):
        """Display total revenue for this event."""
        total = obj.bookings.aggregate(Sum('donation_amount'))['donation_amount__sum'] or 0
        return f"£{total:.2f}"
...
class BookingAdmin(admin.ModelAdmin):
    list_display = ('booking_reference_display', 'event', 'full_name', 'email', 'num_tickets',
                    'donation_amount', 'gift_aid', 'is_paid', 'created_at')
    list_filter = ('event', 'is_paid', 'gift_aid', 'created_at')
    search_fields = ('full_name', 'email', 'id')
    readonly_fields = ('booking_reference_display', 'payment_reference_display', 'created_at', 'updated_at')
    ...
    fieldsets = (
        ...
        ('Booking Information', {
            'fields': ('booking_reference_display', 'full_name', 'email', 'phone_number', 'num_tickets', 'donation_amount')
        }),
        ('Payment Status', {
            'fields': ('is_paid', 'payment_reference_display')
        }),
